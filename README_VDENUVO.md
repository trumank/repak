# VDenuvo Support for Repak

This fork adds support for **VDenuvo** - a custom PAK file encryption used in Unreal Engine 5.5 games with Denuvo anti-tamper protection.

## What is VDenuvo?

VDenuvo is a PAK file format variant (version 11) that uses a custom AES-256 block cipher with non-standard S-boxes and T-tables. It encrypts only the PAK index (not file data), making it different from standard UE5 encryption.

**Tested Game**: Operation Lovecraft: Fallen Doll (UE 5.5)

## Features

- ✅ **Full VDenuvo cipher implementation** - 14-round custom AES-256 variant
- ✅ **Index-only encryption** - File data remains unencrypted per VDenuvo spec
- ✅ **Easy CLI usage** - Simple `--denuvo` flag for all operations
...
All standard repak commands work with the `--denuvo` flag:

### View PAK Info
```bash
repak --denuvo info Pak1.pak
```

### List Files
```bash
repak --denuvo list Pak1.pak
```

### Extract Single File
```bash
repak --denuvo get Pak1.pak "Engine/Config/BaseEngine.ini"
```
...
repak pack --version VDenuvo --mount-point "../../../Paralogue/Config/" ...
```
...
repak pack --version VDenuvo --mount-point "../../../Paralogue/" ...
```
...
```bash
repak --denuvo info original.pak
```
...
- Compare original paths: `repak --denuvo list original.pak`
- With your paths: `repak --denuvo list repacked.pak`
...
- `repak/src/denuvo.rs` - Complete cipher implementation (134KB)
- `repak/src/pak.rs` - Hash calculation fix, encryption logic
- `repak/src/data.rs` - Per-block handling, dynamic sizing
- `repak/src/lib.rs` - VDenuvo version enum, key types
- `repak_cli/src/main.rs` - CLI flag support

## Contributing

This fork is specifically for VDenuvo support. For general repak issues, see the [upstream repository](https://github.com/trumank/repak).
...
- **VDenuvo Support**: Reverse engineering and implementation
